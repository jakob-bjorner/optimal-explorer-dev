...
def extract_solution(solution_str, method="strict"):
    ...
    # el
    if method == "flexible":
        final_answer = "".join(re.findall(r"\d", solution_str)[-3:])
        if len(final_answer) != 3:
            final_answer = None
    else:
        raise Exception(f"Invalid method in extract solution of compute score {method=}")
    return final_answer
...
# def compute_score(solution_str, ground_truth, method="strict", format_score=0.0, score=1.0):
    ...
    method = kwargs.get("method", "flexible")
    ...
    ground_truth = "".join(str(c) for c in ground_truth)
    if answer is None:
        return 0
    else:
