...
# AGENT_MODEL_NAME="o4-mini"
# AGENT_MODEL_NAME="deepseek/deepseek-r1-0528"
FINETUNED_TOKENIZER=false
# AGENT_SAVE_FILE_NAME="Llama-3.1-8b-instruct"
# AGENT_SAVE_FILE_NAME="deepseek-r1-0528"
...
NUM_TRAJECTORIES_PER_GAME_SCENARIO=1 # changed this from 4 for testing with reasoning models.

# Modify the path here to reflect where the paprika directory is located
# cd /path/to/paprika/directory
# agent.model_name=$AGENT_MODEL_NAME agent.tokenizer_name=$AGENT_MODEL_NAME agent.finetuned_tokenizer=$FINETUNED_TOKENIZER agent.save_file_name=$AGENT_SAVE_FILE_NAME
python scripts/games/run_data_generation_game.py seed=$SEED agent=$AGENT env=$ENV judge=$JUDGE game_env=$GAME_ENV game_env.data_type=$DATA_TYPE start_index=$START_INDEX end_index=$END_INDEX  agent.model_max_length=$MODEL_MAX_LENGTH agent_temperature=$AGENT_TEMPERATURE num_trajectories_per_game_scenario=$NUM_TRAJECTORIES_PER_GAME_SCENARIO agent_model_supports_system_message=true