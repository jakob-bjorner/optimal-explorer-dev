...
def load_inference_engine(
    ...
    elif model_type == "openrouter_api_models":
        inference_engine = InferenceEngine.OpenRouterInferenceEngine(
            model_name=model_name,
        )
...
def main(config: DictConfig):
    ...
    for i in tqdm.tqdm(range(config["start_index"], config["end_index"])):
        scenario_history = []
        game_scenario_index = data_indices[i]

        print("\nScenario: ", i - config["start_index"] + 1)
...
# new main function for running *stuff* in parallel and logging json files as soon as they are done. 
# ensuring that they don't overwrite eachother because of the way we have to save to similar file names 
# because we only get to change the prompt by directly editing the game_config json files.
async def alt_main(config: DictConfig):
    """
    Main entry point for running game for data generation.
    """
    config.repo_dir = PARENT_DIR
    ...
    file_version = 0
    new_save_file = str(save_file)
    while os.path.exists(new_save_file):
        new_save_file = save_file.replace(".json", f"_{file_version}.json")
        file_version += 1
    ...
    all_histories = []
    async def easy_runner_get_scenario_history(scenario_num, game_scenario_index):
        scenario_history = []
        ...
        for _ in tqdm.tqdm(range(config["num_trajectories_per_game_scenario"])):
            # Prepare game scenario
            situation_config = {
                "env_input": game_scenarios[game_scenario_index]["env"],
                "agent_input": game_scenarios[game_scenario_index]["agent"],
            ...
            # Run the game through the simulator
            record = await game.run_one_iteration(
                agent_game_scenario=game_scenarios[game_scenario_index]["agent"],
                env_game_scenario=game_scenarios[game_scenario_index]["env"],
                env_first_message=env_first_message,
                agent_first_message=agent_first_message,
            ...
            if record is not None:
                scenario_history.append(record)
        with open(save_file, "a") as fout: # just make them append for easy debugging
            fout.write(json.dumps(scenario_history, indent=1) + "\n")
        return scenario_history
    scenarios = []
    for i in range(config["start_index"], config["end_index"]):
        print("debug", i)
        game_scenario_index = data_indices[i]
        scenario_num = i - config["start_index"] + 1
        scenarios.append(easy_runner_get_scenario_history(scenario_num, game_scenario_index))
    
        # Only record it if we have enough successful responses
    scenario_histories = await asyncio.gather(*scenarios)
