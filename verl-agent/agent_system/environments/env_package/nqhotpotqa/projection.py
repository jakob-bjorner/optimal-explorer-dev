...
def nqhotpotqa_projection(actions: List[str], generate_belief: List[bool]):
    """
    ...
    for i in range(len(actions)):
        if generate_belief[i]:
            ...
            match = extract_first_in_tag('search', actions_og[i])
            if match:
                tags[i] = match[0]
                actions[i] = match[1]
                valids[i] = 1
            else:
                match = extract_first_in_tag('answer', actions_og[i])
                if match:
                    tags[i] = match[0]
                    actions[i] = match[1]
                    valids[i] = 1
