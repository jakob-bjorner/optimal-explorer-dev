...
```bash
python -m vllm.entrypoints.openai.api_server --model meta-llama/Llama-3.1-8B-Instruct --max-model-len 16384 --tensor-parallel-size 1 --gpu-memory-utilization=0.85 --max-num-seqs 16 --port 8000 --enforce-eager --trust-remote-code 
```
...
```bash
CUDA_VISIBLE_DEVICES=1,2,3,4 python scripts/simulate_interactions.py --agent_model meta-llama/Llama-3.1-8B-Instruct \
    --hostname localhost \
    --task_type code \
    --num_tasks 10 \
    --input_path data/backend_tasks/test.jsonl \
    --output_path output/temp_test.jsonl \
    --env_model meta-llama/Llama-3.1-8B-Instruct
python scripts/evaluate_code.py output/temp_test.jsonl
```
...
```bash
python visualizers/visualize_dialogue_histories.py output/temp_test.jsonl
```
#### Frontend Design
