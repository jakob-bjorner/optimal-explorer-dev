    ...
    def invoke_model(self):
        for _ in range(3):
            try:
                ...
                if "oss" in self.model_id.lower():
                    completion = self.client.chat.completions.create(
                        model=self.model_id,
                        messages=messages,
                        max_tokens=1024, # Jakob: this was 4096 before, but shouldn't be more than allowable character limit. Would just take too long.
                        temperature=0,
                        reasoning_effort='low', # reasoning effort seems impossible right now to specify no reasoning.
                    )
                else:
                    completion = self.client.chat.completions.create(
                        model=self.model_id,
                        messages=messages,
                        max_tokens=400, # Jakob: this was 4096 before, but shouldn't be more than allowable character limit. Would just take too long.
                        temperature=0,
                    )
                ...
                # if completion.choices[0].message.content is None:
                #     breakpoint()
                if self.hidden_information.split(',')[0] in completion.choices[0].message.content:
                    continue # we don't want to return the answer, so if its possibly in there, we skip over the generation.
