...
from mdps.wordle import Wordle
...
def get_system_prompt(prompt_style: int) -> str:
    """Get the system prompt based on the prompt style."""
    if prompt_style == 1:
        return """You are playing a wordle game. The rules are:
...
5. Respond with ONLY your 5 letter guess, nothing else."""
        ...
        return """You are playing a wordle game with the goal of optimal exploration. The rules are:
1. Objective - Guess the secret five-letter English word within six attempts.
2. Enter valid words only - Each guess must be a real, spelled-correctly five-letter word.
3. Color feedback after each guess:
   - Green (🟩) - the letter is in the word and in the correct position.
   - Yellow (🟨) - the letter is in the word but in a different position.
   - Gray (⬜) - the letter does not appear in the word at all.
...
5. Important strategy guidelines:
   - Use your first few guesses to explore different letters and positions
   - Pay attention to letter frequency and position patterns
   - Use the feedback to systematically narrow down possibilities
6. Respond with ONLY your 5 letter guess, nothing else."""
...
async def play_single_game(
    ...
    """
    Play a single game of Wordle with the LLM.
    ...
    while True:
        ...
        if not history:
            user_prompt += "Make your first guess (5-letter word):"
        else:
        ...
        guess = ''.join(c for c in llm_response.lower() if c.isalpha())
        if len(guess) != 5 or not mdp._is_valid_guess(guess):
            # If LLM gives invalid response, make a random valid guess
            guess = random.choice(mdp.possible_guess_words)
        ...
        if done:
            ...
            save_game_log(game_id, history, reward == 1.0, mdp.target_word)
...
async def main(
    ...
    tasks = [play_single_game(i, prompt_style, model) for i in range(num_games)]
